...
public class TileRadar extends TileEntityPeripheralBase {
	...
	private AxisAlignedBB getBounds(int d) {
		...
		).expandTowards((double) distance, (double) distance, (double) distance);
	}
	...
	public Object[] getEntities(Context context, Arguments args) {
		...
		double energyNeeded = (Config.COMMON.RADAR_ENERGY_COST_OC.get() * distance * 1.75);
		if(((Connector) node()).tryChangeBuffer(0 - energyNeeded)) {
			AxisAlignedBB bounds = getBounds(distance);
			entities.addAll(RadarUtils.getEntities(getLevel(), getBlockPos(), bounds, PlayerEntity.class));
			entities.addAll(RadarUtils.getEntities(getLevel(), getBlockPos(), bounds, MobEntity.class));
			context.pause(0.5);
		}
	...
	public Object[] getPlayers(Context context, Arguments args) {
		Set<Map> entities = new HashSet<Map>();
		int distance = getDistance(args);
		double energyNeeded = (Config.COMMON.RADAR_ENERGY_COST_OC.get() * distance * 1.0);
		if(((Connector) node()).tryChangeBuffer(0 - energyNeeded)) {
			AxisAlignedBB bounds = getBounds(distance);
			entities.addAll(RadarUtils.getEntities(getLevel(), getBlockPos(), bounds, PlayerEntity.class));
			context.pause(0.5);
	...
	public Object[] getItems(Context context, Arguments args) {
		Set<Map> entities = new HashSet<Map>();
		int distance = getDistance(args);
		double energyNeeded = (Config.COMMON.RADAR_ENERGY_COST_OC.get() * distance * 2.0);
		if(((Connector) node()).tryChangeBuffer(0 - energyNeeded)) {
			AxisAlignedBB bounds = getBounds(distance);
			entities.addAll(RadarUtils.getItems(getLevel(), getBlockPos(), bounds, ItemEntity.class));
			context.pause(0.5);
		}
		return new Object[] { TableUtils.convertSetToMap(entities) };
	}
	...
	// 	return CCRadarProxy.callMethod(getLevel(), getBlockPos(), computer, context, method, arguments, this);
	// }
}
